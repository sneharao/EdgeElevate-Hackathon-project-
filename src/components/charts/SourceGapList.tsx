        ...
        >
          <div className="flex items-start justify-between gap-4 mb-3">
            <div className="flex-1 min-w-0">
              ...
              <div className={`inline-block text-xs font-mono px-2 py-0.5 rounded mt-2 ${getClassificationColor(entry.classification)}`}>
          ...
          <div className="mb-3">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs text-[var(--ink-muted)]">Current Citation Score</span>
              <span className="text-xs font-mono text-[var(--ink)]">{entry.citation_rate.toFixed(2)}</span>
            </div>
          ...
          {entry.competitors_present.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {entry.competitors_present.map((comp, i) => (
                <div
                  key={i}
                  className="text-[10px] px-2 py-1 rounded bg-[var(--line)]/20 text-[var(--ink-muted)]"
                >
                  {comp}
                </div>
              ))}
